...
export class OperationsMonitor {
  ...
  failAIOperation(operationId, error) {
    ...
    // 크리티컬 알림 발송
    this.triggerAlert('ai_operation_failure', {
      operationId,
      type: operation.type,
      error: error.message
    }, AlertSeverity.HIGH);
  ...
  async triggerAlert(ruleName, data, severity = AlertSeverity.MEDIUM) {
    ...
    if (rule.lastTriggered && 
        (now - rule.lastTriggered) < this.performanceConfig.alerts.cooldown) {
      return; // 쿨다운 중
    }
  ...
  getAIOperationsSummary() {
    const recentOps = Array.from(this.operationHistory.values())
      .filter(op => op.endTime > Date.now() - 60 * 60 * 1000); // 최근 1시간
  ...
  cleanupOldMetrics() {
    ...
    // AI 메트릭 정리
    this.metrics.ai.responseTime = this.metrics.ai.responseTime
      .filter(r => r.timestamp > cutoff);
    this.metrics.ai.tokenUsage = this.metrics.ai.tokenUsage
