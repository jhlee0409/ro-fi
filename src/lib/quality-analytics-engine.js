...
export class QualityAnalyticsEngine {
  constructor(platform = null) {
    ...
    this.qualityPatterns = {
      ...
      clichePatterns: [
        '운명적인 만남', '첫눈에 반했다', '심장이 멈췄다',
        '완벽한 남자', '천사같은 미소', '악마같은 매력'
      ]
    };
  ...
  estimateRemainingChapters(novel, criteria) {
    const completionRate = Object.values(criteria).filter(Boolean).length / Object.keys(criteria).length;
  ...
  determineCreativityMode(context) {
    ...
    // 독자 이탈 체크
    const hasReaderIssues = readerData && (
    ...
    // 품질 하락 체크
    const hasQualityIssues = qualityHistory && 
      qualityHistory.slice(-3).every(score => score < 70);
  ...
  calculateWeightedScore(scores) {
    ...
    return Object.entries(scores).reduce(
      (total, [key, score]) => total + (score * (weights[key] || 0)), 0
    );
  }
