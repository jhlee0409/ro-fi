...
export class EpisodeContinuityEngine {
   ...
   * 전체 연속성 검증 (모든 측면)
   */
  async validateAllAspects(storyState, newChapter) {
    const errors = [];
    const warnings = [];
    ...
    // 전체 점수 계산
    const aspectScores = {
      characterConsistency: characterConsistency.confidence,
      worldConsistency: worldConsistency.confidence,
      plotConsistency: plotConsistency.confidence,
      emotionalFlow: emotionalFlow.confidence,
      timelineConsistency: timelineConsistency.confidence
    };
   ...
   * 캐릭터 일관성 검증
   */
  async validateCharacterContinuity(storyState, newChapter) {
    const errors = [];
    const warnings = [];
    ...
    for (const [character, abilities] of newAbilities) {
      ...
      if (prevCharAbilities && !this.areAbilitiesConsistent(prevCharAbilities, abilities)) {
        errors.push({
          ...
          context: { character, previousAbilities, newAbilities }
        });
      }
    }
    ...
    const confidence = errors.length === 0 ? (warnings.length === 0 ? 1.0 .8) .3;
    const valid = errors.filter(e => e.severity === 'critical' || e.severity === 'high').length === 0;

    return { valid, errors, warnings, confidence, aspectScores: { characterConsistency.0, worldConsistency.0, plotConsistency.0, emotionalFlow.0, timelineConsistency.0 } as AspectScores };
  }

  /**
   * 세계관 일관성 검증
   */
  async validateWorldContinuity(storyState, newChapter) {
    const errors = [];
    const warnings = [];
    ...
    const confidence = errors.length === 0 ? (warnings.length === 0 ? 1.0 .8) .4;
    const valid = errors.filter(e => e.severity === 'critical' || e.severity === 'high').length === 0;
   ...
   * 플롯 일관성 검증
   */
  async validatePlotContinuity(storyState, newChapter) {
    const errors = [];
    const warnings = [];
    ...
    const confidence = errors.length === 0 ? (warnings.length <= 1 ? 1.0 .7) .3;
    const valid = errors.filter(e => e.severity === 'critical' || e.severity === 'high').length === 0;
   ...
   * 감정선 연속성 검증
   */
  async validateEmotionalFlow(storyState, newChapter) {
    const errors = [];
    const warnings = [];
    ...
    if (!previousChapter) {
      return { valid, errors: [], warnings: [], confidence.0, aspectScores: { characterConsistency.0, worldConsistency.0, plotConsistency.0, emotionalFlow.0, timelineConsistency.0 } as AspectScores };
    }
    ...
    const confidence = errors.length === 0 ? (warnings.length === 0 ? 1.0 .8) .5;
    const valid = errors.filter(e => e.severity === 'critical' || e.severity === 'high').length === 0;
   ...
   * 타임라인 일관성 검증
   */
  async validateTimelineContinuity(storyState, newChapter) {
    const errors = [];
    const warnings = [];
    ...
    const confidence = errors.length === 0 ? 1.0 .3;
    const valid = errors.filter(e => e.severity === 'critical' || e.severity === 'high').length === 0;
   ...
   * 연속성 수정 제안 생성
   */
  async suggestContinuityFix(validationErrors, _storyState)<FixSuggestion[]> {
    const suggestions = [];
      ...
      switch (error.type) {
        case 'CHARACTER_NAME_CHANGED':
          suggestions.push({
            ...
            changes: [{
              ...
              oldContent.context.newName,
              newContent.context.previousName,
        ...
        case 'WORLD_RULE_VIOLATION':
          suggestions.push({
            ...
            changes: [{
              file: `chapters/novel-*-ch${error.chapterNumber}.md`,
              oldContent.context.violatingContent,
              newContent.suggestedFix || '세계관 규칙에 맞게 수정',
              reason: '세계관 일관성 유지'
            }],
            confidence.7
          });
   ...
   */
  private getChapterContent(_chapter) {
   ...
   * 캐릭터 이름 추출
   */
  private extractCharacterNames(_content) {
   ...
   * 주인공 식별
   */
  private identifyProtagonist(characters, _content) | null {
   ...
   * 캐릭터 능력 추출
   */
  private extractCharacterAbilities(_content)<string, string[]> {
   ...
   * 능력 일관성 검사
   */
  private areAbilitiesConsistent(prevAbilities, newAbilities) {
   ...
   * 성격 변화 감지
   */
  private detectPersonalityChanges(prevChapter, newChapter)<{character, description}> {
    // 간단한 구현 - 실제로는 더 정교한 NLP 분석 필요
    const changes<{character, description}> = [];
   ...
   * 마법 시스템 위반 검사
   */
  private checkMagicSystemViolations(_magicSystem, _content) {
    const errors = [];
   ...
   * 세계관 규칙 위반 검사
   */
  private checkWorldRuleViolations(rules, _content) {
    const errors = [];
   ...
   * 지리적 일관성 검사
   */
  private checkGeographyConsistency(geography, _content) {
    const warnings = [];
   ...
   * 사회적 계층 일관성 검사
   */
  private checkSocialHierarchyConsistency(_hierarchy, _content) {
    const warnings = [];
   ...
   * 플롯 구멍 감지
   */
  private detectPlotHoles(storyState, newChapter) {
    const errors = [];
   ...
   * 이벤트 순서 검증
   */
  private checkEventSequence(timeline, newChapter) {
    const errors = [];
    ...
    if (lastEvent && newChapter.chapterNumber < lastEvent.chapterNumber) {
      errors.push({
        ...
        context: { lastEventChapter.chapterNumber, currentChapter.chapterNumber }
      });
    }
   ...
   * 감정적 거리 계산
   */
  private calculateEmotionalDistance(emotion1, emotion2) {
   ...
   * 감정 점수 계산
   */
  private getEmotionalScore(emotion) {
