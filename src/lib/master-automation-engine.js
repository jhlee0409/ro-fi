...
export class MasterAutomationEngine {
  ...
  async generateChapter(novelSlug, chapterNumber, concept, characters, isFirst = false, emotionStage = null) {
    // 기본 챕터 구조
    const baseContent = {
      title: isFirst ? "첫 만남" : `${chapterNumber}화`,
      novel: novelSlug,
      chapterNumber,
  ...
  // 스토리 내용 생성 - NOVEL_MARKDOWN_FORMAT.md 준수
  generateStoryContent(concept, chapterNumber, emotionalElements, emotionStage) {
    const samples = [
...
${emotionalElements.sensoryDetail}이 두 사람을 감쌌다.`
    ...
    return samples[chapterNumber % samples.length];
  }
