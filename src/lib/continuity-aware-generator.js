 ...
 * 기능, EpisodeContinuityEngine, ContextWindowManager 통합하여 연속성 보장
 */
...
// import { GeminiResponse } from './types/api.js';
...
export class ContinuityAwareEpisodeGenerator {
   ...
   * 5. 성능 최적화 적용
   */
  async generateNextChapter(novelSlug) {
    ...
    try {
      ...
      return {
        ...
        metadata: {
          ...
          continuityScore.continuityScore || 0,
          operationId,
        },
        context,
        ...
        if (recoveryResult?.content) {
          ...
          return {
            ...
            metadata: {
              ...
              validationPassed,
              attempts,
              qualityScore: (recoveryResult as { qualityScore? })?.qualityScore || 6.0,
              continuityScore.6,
              operationId,
              recoveryMode,
            },
   ...
   * 🧠 사전 생성 분석 수행
   */
  private async performPreGenerationAnalysis(novelSlug) {
    try {
      // 시스템 상태 분석
      ...
      return {
        ...
        performanceMetrics: { systemEfficiency.8 },
      };
    }
  }
...
위 기준을 모두 충족하는 최고 품질의 콘텐츠를 작성해주세요.`;
  ...
  private async generateWithQualityAssurance({
    ...
    for (let attempt = 1; attempt <= this.WORLD_CLASS_CONFIG.MAX_GENERATION_ATTEMPTS; attempt++) {
      try {
        ...
        const qualityScore = { overall.overallScore };
  ...
  private async updatePerformanceMetrics(
    ...
    if (success && result && typeof result === 'object') {
      ...
      const resultObj = result as { qualityScore?; continuityScore? };
   ...
   * 🆘 Fallback 챕터 생성
   */
  private createFallbackChapter(chapterNum, content) {
    return {
      chapterNumber,
      title: `${chapterNum}화`,
...
`;
   ...
   * Gemini API 호출
   */
  private async callGeminiWithContext(prompt, _context) {
    try {
  ...
  private parseGeneratedContent(
    ...
    try {
      ...
      const chapterState = {
        ...
        summary.summary || '',
        keyEvents.keyEvents || [],
        characterStates Map(Object.entries(parsed.characterStates || {})),
        newCharacters: [],
        locationChanges Map(),
        emotionalTone.emotionalTone || 'neutral',
        endingEmotionalState.endingEmotionalState || parsed.emotionalTone || 'neutral',
        cliffhanger.cliffhanger,
        plotProgression: {
          mainArcProgress.plotProgression?.mainArcProgress || '',
          subplotChanges: [],
          foreshadowingPlanted.plotProgression?.foreshadowingPlanted || [],
          foreshadowingResolved.plotProgression?.foreshadowingResolved || [],
        },
   ...
   * 검증 피드백 생성
   */
  private generateValidationFeedback(validationResult) {
    const feedback = [];
   ...
   * 프롬프트에 검증 피드백 추가
   */
  private addValidationFeedback(originalPrompt, feedback) {
...
위 피드백을 모두 반영하여 다시 작성해주세요.`;
   ...
   * 토큰 사용량 추정
   */
  private estimateTokenUsage(prompt) {
    // 한국어 기준 대략적 추정
   ...
   */
  async generateMultipleChapters(novelSlug, count)<GenerationResult[]> {
    const results = [];
   ...
   */
  async getGenerationStats(novelSlug)<{
    totalChapters;
    avgWordCount;
    avgGenerationTime;
    ...
    const worldClassCompliance = stabilityIndex >= 0.85 ? 1.0  / 0.85;

    return {
      totalChapters.length,
      avgWordCount,
      avgGenerationTime.performanceMetrics.averageResponseTime,
      validationSuccessRate,
      continuityScore,
      qualityScore,
      stabilityIndex,
