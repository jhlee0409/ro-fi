...
export class StoryStateManager {
  ...
  private stateDirectory = './data/story-states/';
   ...
   * 새 소설 초기화
   */
  async initializeStory(novelSlug: string, config: {
    title: string;
    author: string;
    genre: string;
    tropes: string[];
   ...
   * 스토리 상태 로드
   */
  async getStory(novelSlug: string): Promise<StoryState> {
    // 메모리에서 확인
    ...
    try {
      ...
      // Map 객체들 복원
      storyState.characters.main = new Map(storyState.characters.main as any);
      storyState.characters.supporting = new Map(storyState.characters.supporting as any);
      storyState.characters.minor = new Map(storyState.characters.minor as any);
      storyState.chapters = new Map(storyState.chapters as any);
      storyState.continuity.characterStates = new Map(storyState.continuity.characterStates as any);
      storyState.continuity.locationStates = new Map(storyState.continuity.locationStates as any);
      storyState.worldbuilding.geography.locations = new Map(storyState.worldbuilding.geography.locations as any);
   ...
   * 스토리 상태 저장
   */
  async saveStoryState(novelSlug: string): Promise<void> {
    ...
    const serializableState = {
      ...storyState,
      characters: {
        main: Array.from(storyState.characters.main.entries()),
        supporting: Array.from(storyState.characters.supporting.entries()),
        minor: Array.from(storyState.characters.minor.entries())
      },
      chapters: Array.from(storyState.chapters.entries()),
      continuity: {
        ...storyState.continuity,
        characterStates: Array.from(storyState.continuity.characterStates.entries()),
        locationStates: Array.from(storyState.continuity.locationStates.entries())
      },
      worldbuilding: {
        ...storyState.worldbuilding,
        geography: {
          ...storyState.worldbuilding.geography,
          locations: Array.from(storyState.worldbuilding.geography.locations.entries())
        }
      }
    };
   ...
   * 챕터 추가 후 상태 업데이트
   */
  async updateAfterChapter(novelSlug: string, chapterState: ChapterState): Promise<void> {
    const story = await this.getStory(novelSlug);
    
    // 챕터 기록
    story.chapters.set(chapterState.chapterNumber, chapterState);
    story.metadata.currentChapter = Math.max(story.metadata.currentChapter, chapterState.chapterNumber);
    ...
    for (const event of chapterState.keyEvents) {
      story.continuity.timeline.push({
        chapterNumber: chapterState.chapterNumber,
        event,
        timestamp: new Date().toISOString(),
        participants: Array.from(chapterState.characterStates.keys()),
        significance: 'medium'
      });
    }
    ...
    for (const foreshadowing of chapterState.plotProgression.foreshadowingPlanted) {
      story.plotProgress.foreshadowing.push({
        ...
        planted: chapterState.chapterNumber,
        resolved: false
      });
    }
   ...
   */
  async analyzeExistingContent(novelSlug: string): Promise<StoryState> {
    try {
      // 챕터 컨텐츠 로드
   ...
   * 챕터 컨텐츠 분석하여 상태 추출
   */
  private async analyzeChapterContent(chapter: any): Promise<ChapterState> {
    ...
    const chapterState: ChapterState = {
      ...
      emotionalTone: emotions.tone,
      endingEmotionalState: emotions.ending,
      cliffhanger: this.detectCliffhanger(content),
      plotProgression: {
        mainArcProgress: '',
        subplotChanges: [],
      ...
      wordCount: chapter.data.wordCount || content.length,
      contentRating: chapter.data.contentRating || '15+',
      publishedDate: new Date(chapter.data.publishedDate)
    };
  ...
  private extractKeyEvents(content: string): string[] {
    ...
    for (const sentence of sentences) {
      if (sentence.includes('말했다') || sentence.includes('소리쳤다') || 
          sentence.includes('속삭였다') || sentence.includes('대답했다')) {
        events.push(sentence.trim().substring(0, 50) + '...');
      } else if (sentence.includes('마법') || sentence.includes('검') || 
                 sentence.includes('공격') || sentence.includes('구했다')) {
        events.push(sentence.trim().substring(0, 50) + '...');
      }
    }
   ...
   * 감정 톤 분석
   */
  private analyzeEmotionalTone(content: string): { tone: string; ending: string } {
    ...
    let positive = 0, negative = 0, tension = 0;
    
    positiveWords.forEach(word => positive += (content.match(new RegExp(word, 'g')) || []).length);
    negativeWords.forEach(word => negative += (content.match(new RegExp(word, 'g')) || []).length);
   ...
   * 클리프행어 감지
   */
  private detectCliffhanger(content: string): string | undefined {
    const lastSentences = content.split(/[.!?]/).slice(-3);
    
    for (const sentence of lastSentences) {
      if (sentence.includes('그때') || sentence.includes('갑자기') || 
          sentence.includes('하지만') || sentence.includes('...')) {
   ...
   * 모든 활성 스토리 목록
   */
  async getActiveStories(): Promise<string[]> {
    try {
   ...
   * 스토리 삭제
   */
  async deleteStory(novelSlug: string): Promise<void> {
    ...
    try {
      ...
      console.warn(`스토리 파일 삭제 실패: ${novelSlug}`, error);
    }
  }
}
